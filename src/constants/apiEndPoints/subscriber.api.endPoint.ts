...
export class SubscriberApiEndPoint {
    ...
    GET_WITH_EMAIL(email: string) { return PathUtil.createPath(this.mainPath, `/get/email/${email}`); }
    ...
    DELETE_WITH_ID(_id: string) { return PathUtil.createPath(this.mainPath, `/delete/${_id}`); }
    DELETE_WITH_EMAIL(email: string) { return PathUtil.createPath(this.mainPath, `/delete/email/${email}`); }
